...
CREATE TABLE `forums` (
  ...
  `descr` varchar(150) DEFAULT NULL COMMENT 'Description of the forum',
...
CREATE TABLE `posts` (
  ...
  `revision` int(10) unsigned NOT NULL DEFAULT 1 COMMENT 'The current text revision of the post.',
  `ip` char(15) NOT NULL COMMENT 'IP address of the poster.',
  `deleted` tinyint(3) unsigned NOT NULL DEFAULT 0 COMMENT 'Is post deleted?',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `users` (
  ...
  `ppp` tinyint(3) unsigned DEFAULT NULL COMMENT 'Posts per page for pagination',
  `tpp` tinyint(3) unsigned DEFAULT NULL COMMENT 'Threads per page for pagination',
