...
CREATE TABLE `forums` (
  ...
  `minread` tinyint(4) NOT NULL DEFAULT -1 COMMENT 'Minimum rank to read/view forum',
  `minthread` tinyint(4) NOT NULL DEFAULT 1 COMMENT 'Minimum rank to make new threads',
  `minreply` tinyint(4) NOT NULL DEFAULT 1 COMMENT 'Minimum rank to reply to threads',
  PRIMARY KEY (`id`),
  KEY `cat` (`cat`),
...
CREATE TABLE `users` (
  ...
  `rank` tinyint(4) NOT NULL DEFAULT 1 COMMENT 'User''s rank, controlling access and permissions. (see perm.php)',
