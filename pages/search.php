...
if ($query && $where == 1) {
	...
	$posts = query("SELECT $userfields $fieldlist p.*, p.text, p.editdate ptdate, p.revision cur_revision, t.id tid, t.title ttitle, t.forum tforum
			FROM posts p
			JOIN users u ON p.user = u.id
			JOIN threads t ON p.thread = t.id
			JOIN forums f ON f.id = t.forum
			WHERE p.text LIKE CONCAT('%', ?, '%') AND ? >= f.minread
